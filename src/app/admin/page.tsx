...
import { getStage, saveStage, stages, getTodayStage } from "@/lib/data";
...
export default function AdminPage() {
  ...
  const handleAuto = () => { 
    if (typeof window !== "undefined") localStorage.removeItem("rynkeby_stage_override");
    setStage(getTodayStage()); 
    setSaved(false); 
  };
  ...
  return (
    <div className="page">
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {stages.map((s) => (
            <button key={s.stageNumber} onClick={() => { setStage(s); setSaved(false); }} style={{
              ...
              background: stage.stageNumber === s.stageNumber ? "rgba(200,16,46,0.15)" : "rgba(255,255,255,0.05)",
              color: "var(--text)", fontSize: "0.8rem", cursor: "pointer",
            }}>S{s.stageNumber}</button>
          ))}
        </div>
      ...
      <div style={{ marginTop: "1rem" }}>
        <button className="btn btn-red" onClick={handleSave} style={{ 
          marginBottom: "0.75rem", 
          background: saved ? "rgba(16,185,129,0.2)" : "#C8102E", 
