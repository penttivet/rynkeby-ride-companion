...
function MemberCard({
  ...
  if (editing) {
    return (
      <div style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(200,16,46,0.3)", borderRadius: 12, padding: "1rem", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
        <p style={{ color: "#fff", fontWeight: 700, margin: 0, fontSize: "0.9rem" }}>✏️ Muokkaa tietoja</p>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Nimi" style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.05)", color: "#fff", fontSize: "0.9rem" }} />
        <input value={role} onChange={e => setRole(e.target.value)} placeholder="Rooli" style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.05)", color: "#fff", fontSize: "0.9rem" }} />
        <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Puhelin (+358...)" style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.05)", color: "#fff", fontSize: "0.9rem" }} />
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          {photo && <img src={photo} alt="preview" style={{ width: 48, height: 48, borderRadius: "50%", objectFit: "cover" }} />}
          <button onClick={() => fileRef.current?.click()} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#8b949e", cursor: "pointer", fontSize: "0.85rem" }}>
            📷 {photo ? "Vaihda kuva" : "Lisää kuva"}
          </button>
          <input ref={fileRef} type="file" accept="image/*" onChange={handlePhoto} style={{ display: "none" }} />
        </div>
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <button onClick={handleSave} disabled={saving} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "none", background: "#C8102E", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: "0.9rem", opacity: saving ? 0.6 : 1 }}>
            {saving ? "Tallennetaan..." : "Tallenna"}
          </button>
          <button onClick={() => setEditing(false)} style={{ padding: "10px 14px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#8b949e", cursor: "pointer", fontSize: "0.85rem" }}>Peruuta</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "0.85rem 1rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
      {member.photo ? (
      ...
      <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
        ...
        {canEdit && (
          <>
            <button onClick={() => setEditing(true)} style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(255,200,0,0.15)", border: "1px solid rgba(255,200,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.9rem", cursor: "pointer" }}>✏️</button>
...
function TeamCard({ team }: { team: Team }) {
  ...
  const handleUpdate = async (updated: TeamMember) => {
    try {
      await fetch("/api/team/members", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ teamId: team.id, memberId: updated.id }),
      });
      await fetch("/api/team/members", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ teamId: team.id, member: updated }),
      });
    ...
    <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${open ? "rgba(200,16,46,0.25)" : "rgba(255,255,255,0.08)"}`, borderRadius: 14, overflow: "hidden" }}>
      ...
      {open && (
        <div style={{ padding: "0 1rem 1rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {loading ? (
            <p style={{ color: "#8b949e", textAlign: "center", fontSize: "0.85rem" }}>Ladataan...</p>
          ) : members.length > 0 ? (
            members.map(m => <MemberCard key={m.id} member={m} canEdit={loggedIn} onDelete={handleDelete} onUpdate={handleUpdate} />)
          ) : (
          ...
          {showLogin && !loggedIn && (
            <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "0.75rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                ...
                <button onClick={() => { setShowLogin(false); setPwError(""); setPassword(""); }} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#8b949e", cursor: "pointer", fontSize: "0.85rem" }}>Peruuta</button>
              </div>
            </div>
          )}
